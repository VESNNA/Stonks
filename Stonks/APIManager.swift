        ...
        let dataTask = JSONTaskWith(request: request) { (json, response, error) in
            DispatchQueue.main.async(execute: {
                ...
                if let value = parse(json) {
                    ...
                    let userInfo = [
                        NSLocalizedDescriptionKey: NSLocalizedString("JSON processing error", comment: "")
                    ]
                    let error = NSError(domain: EXMPLNetworkingErrorDomain, code: 102, userInfo: userInfo)
                    completionHandler(.Failure(error))
                }
